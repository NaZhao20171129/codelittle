...
public:
  ...
  inline double getFitness(const int index) const {return guys[index]->getFitness();}
  Individual *getBestIndividual(void) {return bestInd; }
...
public:
  ...
  void quickSort(NsgaIndividual **theGuys, int *output, int left, int right, int objID);
