...
public:  
  ...
  inline void setViolation(int index, double newViolation) {violation[index] = newViolation;}
  ...
  inline void setObjective(double newObjective) {if (globalSetup->gaType==SGA) *objFunction = newObjective; else exit(0);}
  inline void setFitness(double newFitness) {if (globalSetup->gaType==SGA) *fitness = newFitness; else exit(0);}
  ...
  inline void flood(int index) {
    double value;	 
    value = myRandom.boundedRandom(globalSetup->variableRanges[index][0],globalSetup->variableRanges[index][1]);
    if(globalSetup->variableTypes[index] == Integer)
      value = int(value+0.5);
    chrom.setValue(index,value);
  }
  void evaluateFitness(void);
  void loadIndividual(double *varValues, double *objValues, double *constViolValues, double penaltyValue);
  friend bool constrTournComp (const Individual *guy1, const Individual *guy2, int compareWhat);
...
public:
  ...
  friend bool crowdingComp(const NsgaIndividual *guy1, const NsgaIndividual *guy2);
