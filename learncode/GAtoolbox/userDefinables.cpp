...
int main(int argc, char *argv[]) {
  ...
  if ((globalSetup->localSearchMethod != NoLocalSearch) && (globalSetup->gaType == NSGA)) {
    fclose(fInput);
  ...
  if(globalSetup->noOfStoppingCriterias > 0) {
    ...
    globalSetup->otherStoppingCriteria = new StoppingCriterias[globalSetup->noOfStoppingCriterias];
    globalSetup->stoppingParameter = new double[globalSetup->noOfStoppingCriterias];
	...
	break;
      ...
      case BestFitness:
	// 2 - SGA w/o MM only
	if ((globalSetup->gaType != SGA) || (globalSetup->nichingType != NoNiching)) {
	  fclose(fInput);
