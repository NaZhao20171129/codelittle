...
TournamentSelection::TournamentSelection (const int size, const Population *pop) :
  Selection(pop) , tournamentSize(size)
{
}

TournamentSelectionWithReplacement::TournamentSelectionWithReplacement (const int size, const Population *pop) :
  Selection(pop) , tournamentSize(size)
{
}

StochasticUniversalSelection::StochasticUniversalSelection (const Population *pop) :
  Selection(pop)
{
}
...
/*** 
Tournament Selection Without Replacement: Randomly select S individuals without replacement and copy the best individual to the mating pool. Repeat this process till N individuals are selected. 
Implementation: 
...
Note: Tournament selection without replacement should be preferred over tournament selection with replacement.
*/
...
{                                       
  ...
  for(ii = 0; ii < tournamentSize; ii++) {
	...
	if(betterIndividual((*pop)[p1],(*pop)[p2])) winner[ll] = p1;
	else winner[ll] = p2;
	p1 = winner[ll];
      }
      ++ll;
...
/*** 
Tournament Selection With Replacement: Randomly select S individuals with replacement and copy the best individual to the mating pool. Repeat this process till N individuals are selected. 
Implementation: 
   1. Shuffle the Individuals of the current population
...
/*** 
Roulette Wheel Selection: Each individual is assigned a slice proportional to its fitness. The wheel is spun N times, where N is the number of individuals in the population. 

Note: Ordinal selection schemes like tournament or trunction selection schemes should always be preferred over roulette wheel selection
Note: Stochastic universal selection should be preferred over roulette wheel selection.
Note: Roulette wheel selection should always be used with some scaling method.
*/
