...
/***
Crowding comparison operator: Determines if NsgaIndvidual 1 is better than NsgaIndividual 2.
If the rank of individual 1 is less than that of individual 2, then return 1. If the rank of individual 1 and 2 are same and the crowding distance of individual 1 is greater than that of individual 2 then return 1. For all other cases return 0.
*/
...
/***
Constraint tournament comparison operator: Determines if the individual 1 is better than individual 2.
The flag compareWhat selects whether the objective value or the fitness is to be compared. 
If objective value is to be compared and we are required to maximise the objective function then the fitness is set to be equal to the objective value. 
On the other hand if the objective function is to be minimized, then the fitness is set to negative of objective function.
If both the individuals are feasible or if they have the same amount of infeasibility (quantified by total constraint violation), then return 1 if the fitness
of individual 1 is greater than that of individual 2. 
On the other hand if individual 1 is feasible and individual 2 is infeasible then return 1.
If both individuals are infeasible but individual 1 has lower infeasibility (lower constraint violation value) then return 1.
For all other cases return 0
 */
bool constrTournComp (const Individual *guy1, const Individual *guy2, int compareWhat) 
{
...
If the constraint handling method is through tournament selection, then
  -- If both individuals are feasible or have the same amount of infeasibility, then if every fitness value of individual 1 is greater than or equal to the fitness value of individual 2 then return 1. 
  -- If indiviudal 1 is feasible and individual 2 is infeasible return 1.
  -- If both individuals are infeasible, but the infeasiblity of individual 1 is lesser than individual 2 then return 1
  -- If both individuals are feasible or have the same amount of infeasibility, then if every fitness value of individual 2 is greater than or equal to the fitness value of individual 1 then return -1. 
  -- If indiviudal 2 is feasible and individual 1 is infeasible then return -1
  -- If both individuals are infeasible, but the infeasiblity of individual 2 is lesser than individual 1 then return -1.
  -- For all other cases return 0.
...
/// Penaly constraint comparison operator: Checks if indiviudal 1 is better than individual 2 if the constraint handling is through penalty method.
...
///Individual comparison operator: Determines if individual 1 is better than individual 2 when no constraints are present.
