...
Chromosome::Chromosome(void) {
  ...
  for (ii = 0; ii < globalSetup->noOfDecisionVariables; ii++) {
    genes[ii] = myRandom.boundedRandom(globalSetup->variableRanges[ii][0],globalSetup->variableRanges[ii][1]);
...
void Chromosome::mutateMinMax (const int *freezeMask) {
  ...
  if(((globalSetup->gaType == SGA)&&(freezeMask[position] == OFF))||(globalSetup->gaType == NSGA)) {
    ...
    if(myRandom.flip(globalSetup->mutationProbability)) {
      ...
      genes[position] = myRandom.boundedRandom(globalSetup->variableRanges[position][0],globalSetup->variableRanges[position][1]);
...
void Chromosome::mutateNormal(const int *freezeMask) {
  ...
  for (ii = 0; ii < globalSetup->noOfDecisionVariables; ii++) {
    ...
    if(((globalSetup->gaType == SGA)&&(freezeMask[ii] == OFF))||(globalSetup->gaType == NSGA)) {
	...
	if(genes[ii] > globalSetup->variableRanges[ii][1]) genes[ii] = globalSetup->variableRanges[ii][1];
	if(genes[ii] < globalSetup->variableRanges[ii][0]) genes[ii] = globalSetup->variableRanges[ii][0];
      }
    }
  }
}
  ...
  for(ii = 0; ii < globalSetup->noOfDecisionVariables; ii++) {
	...
	if(genes[ii] > geneMin) {
	  ...
	  if(rndNo <= 0.5) {
	    ...
	    value = 2.0*(1.0-rndNo) + 2.0*(rndNo - 0.5)*pow(1.0-delta,(double)(etaM+1));
	...
	if(globalSetup->variableTypes[ii] == Integer) genes[ii] = int(genes[ii]+0.5);
...
void Chromosome::mutatePolynomial(const int index) {
  ...
  if(globalSetup->variableTypes[index] == Integer) genes[index] = int(genes[index]+0.5);
