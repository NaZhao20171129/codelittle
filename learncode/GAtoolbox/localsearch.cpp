...
The number of decision variables for the local search (Nvl) equals the number of decision variables minus the number of frozen variables. The initial Nvl+1 individuals are generated as follows: One of the individuals is the individual provied by GA, the others are generated by mutating one of the unfrozen genes.
* Find the best, worst, and the next to worst individuals. 
* Reflect the worst individual along the centroid. 
* If the reflected individual is better than the best individual, then expand the reflected individual.
* If the reflected individual is worse than the next to worst individual, then contract the reflected individual.
* If the contracted point is worse than the worst individual then reflect along the best individual
* Repeat the above four steps till convergence is reached or the maximum number of allowed evaluations is exhausted.
This function returns the total number of function evaluations taken by the local search.
Reference: Press, W., Flannery, B., Teukolsky, S., & Vettering, W. (1989). Numerical Recipes in C. Cambridge: Cambridge University Press. http://lib-www.lanl.gov/numerical/bookcpdf/c10-4.pdf. (http://www.nr.com).
 */
int Simplex::localSearcher(Individual *theGuy, Individual *localHero, int *freezeMask)
{
  ...
  *localGuys[0] = *theGuy;
  ...
  for(;;) {
    ...
    tolerance = 2.0*fabs(localGuys[worst]->getObjective()-localGuys[best]->getObjective())/
      (fabs(localGuys[worst]->getObjective())+fabs(localGuys[best]->getObjective())+ZERO);
...
/***
Simplex operator: reflect, expands and contracts the worst individual. If the resultant individual is better than the earlier one than replaces the worst individual. Also returns the next operation to be done.
 */
