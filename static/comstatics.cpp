...
//const int duanbiao[duanshu+1]={0,26,35,67,78,96};
const int duanbiao[duanshu+1]={0,30,70,96};  //good 2
using namespace std;
...
int main()
{
	...
	for(int t=0;t<4;t++)
	{
		...
		for(int j=0;j<duanshu;j++)
		{
			...
			stdl_duan.clear();
			for(int k=duanbiao[j];k<duanbiao[j+1];k++)
			{
				if(k==95)
					cout<<stdl[k]<<endl;
				stdl_duan.push_back(stdl[k]);
			}
			commetric(stdl_duan,out);
		}
		out<<endl;
	}
