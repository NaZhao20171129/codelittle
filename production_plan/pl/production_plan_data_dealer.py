...
def readcontent(filename):
        workbook = xlrd.open_workbook(os.getcwd()+'/draw1/'+filename)
	    ...
	    #plt.show()
	    ylim(0,max*1.02)
	    print filename[0:12].replace(' ',''),'a'
	    filewt=filename[0:12].strip()+"s"+str(count)+".png"
	    print filewt
            plt.savefig(filewt, format='png')
            count+=1
	    #break
...
def count_and_finished(context,startline,table,number,year):
        ...
        for row in range(startline,startline+PART_NUM):
                if ((row-startline==7) or (row-startline==8) or (row-startline==9)):
                    print 'row=%d' %row
                    new_fin_num=(float(context[row][5])*float(context[row][8])+float(context[row][4])*float(context[row][9]))
                    finished_num=num_finished_list[row-4-startline]+new_fin_num
                    num_finished_list[row-4-startline]=finished_num
                    count_list[row-4-startline]+=context[row][3]	
                    continue
	...
	first=plt.bar(x,count_list,align='center',color=col,label=suodui,width=0.18)
	xlim(-0.2,9.3)
...
#	first=plt.bar(x,count_list,align='center',label=suodui)
	...
	for a,b in zip(x,count_list):
		    text(a, b+0.05, '%d' % b, ha='center', va= 'bottom',size=8)
...
def main():
	for f in os.listdir(os.getcwd()+"/draw1"):
		filename=f
		print filename
		try:
