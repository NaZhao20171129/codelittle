...
def count_and_finished(context,startline,table,number,year):
	...
	money_list=[]
        ...
        for row in range(startline,startline+PART_NUM):
			...
			new_count_per=(float(context[row][3])*float(context[row][4])+float(context[row-4][3])*float(context[row-4][4]))/(float(context[row-4][3])+float(context[row][3]))
		    except ZeroDivisionError:
		    	new_count_per=0.0
		    count_list[row-4-startline]=new_count_per	
                    continue
